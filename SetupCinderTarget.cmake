...
function( ci_setup_app )
	set( oneValueArgs APP_NAME CINDER_PATH ASSETS_PATH )
	set( multiValueArgs RESOURCES BLOCKS )
...
#	endif()
	...
	if( CINDER_MAC )
		...
		if(ARG_RESOURCES)
			target_add_sources(${ARG_APP_NAME} ${ARG_RESOURCES})
		endif()
	elseif( CINDER_LINUX )
		if( ARG_RESOURCES )
			# copy resources to a folder next to the app names 'resources'. note the folder is flat, so duplicates will be overwritten
			get_filename_component( RESOURCES_DEST_PATH "${CMAKE_CURRENT_BINARY_DIR}/resources" ABSOLUTE )
	...
	get_filename_component( ASSETS_DEST_PATH "${CMAKE_CURRENT_BINARY_DIR}/assets" ABSOLUTE )
	if( EXISTS "${ARG_ASSETS_PATH}" AND IS_DIRECTORY "${ARG_ASSETS_PATH}" )
		...
		if( CINDER_COPY_ASSETS )
			...
			get_filename_component( ASSETS_DEST_PATH "${CMAKE_CURRENT_BINARY_DIR}" ABSOLUTE )
